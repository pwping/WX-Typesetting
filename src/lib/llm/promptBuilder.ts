...
export async function buildTypesetPrompt(
  ...
  const systemPrompt = skillCore + "\n\n" +
    ...
    "  - **所有组件的内容区左右边距统一为 8px**（即 padding:0 8px 或 margin:0 8px），确保文章内容在手机端两侧留白适中\n\n" +
    "输出前请逐条检查：\n" +
    ...
    "6. 正文段落 font-size 是否为 15px？所有正文段落必须统一覆盖为 15px\n" +
    "7. Markdown 中的 `##` 二级标题是否全部使用了主题组件库中的二级标题组件（如 chapter-title）渲染？**逐行比对 Markdown 源文中每一行 `## ` 开头的标题**，确保每一个都对应输出了一个 chapter-title 组件。**绝对禁止用 `**加粗文字**` 替代章节标题**——如果输出 HTML 中出现 `**xxx**` 形式且内容与 Markdown 中某个 `## 标题` 一致，说明该标题被错误地降级为加粗段落，必须改为 chapter-title 组件\n" +
    "8. **加粗/下划线微信安全规则**：`**加粗**`必须渲染为 `<span style=\"font-weight:700;\"><span leaf=\"\">加粗</span></span>`；下划线标记（Markdown 无原生语法，用 `<u>` 标识）必须渲染为 `<span style=\"border-bottom:2px solid 主题主色;font-weight:600;\"><span leaf=\"\">文字</span></span>`。禁止使用 `text-decoration:underline`（微信会丢失）\n" +
    "9. HTML 最后必须在所有组件闭合后添加一个空段落：`<p style='margin:0;'><span leaf=''><br></span></p>`，确保在公众号编辑器末尾有一个可点击输入的空白行，方便继续添加内容\n" +
    "10. **章节编号严格按 `##` 出现顺序连续递增**（01/02/03…），不得跳号、不得根据章节内容字面意思判断末章。末章编号变体（`///`、`∞`、`LAST`）**只能用于 Markdown 中实际最后一个 `##` 标题**——即使某章标题含「结语/总结/写在最后」字眼，只要后面还有其他 `##` 标题，就只能使用数字编号\n" +
    "11. **每个 `##` 标题都是独立章节**：禁止合并相邻的 `##` 标题，禁止省略任何一个 `##` 标题。`## 数据` 和 `## 隐私` 必须保留为两个独立章节，不能合并成「数据隐私」" +
    noImageRule
