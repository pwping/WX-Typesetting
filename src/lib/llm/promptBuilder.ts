...
export async function buildCustomThemePrompt(
  ...
  const systemPrompt = genPrompt + "\n\n" +
    ...
    "### 1. 设计变量速查表\n" +
    "用 ` ``` ` 代码围栏列出：主色调 / 辅色 / 标题色 / 正文色 / 次要文字 / 注释色 / 分割线色 / 浅底 / 浅边框 / 字号(正文必须16px，不可改) / 行高 / 间距 / max-width(677px) / 内容区边距(padding:0 10px) / TOC（取值：横向卡片 | 无）。\n\n" +
    ...
    "是否包含目录组件：分析设计风格后决定——信息密度高的主题（教程/测评/科技/数据/商务/工具盘点）需要目录；极简随笔/禅意/文学类可省略。如果需要目录，选择一个明确的布局方向（只能是横向卡片滚动），禁止在同一次生成中使用两种。这个决策必须在设计变量速查表中以 `TOC: 横向卡片 | 无` 的形式明确记录。\n" +
    "每个带图片槽位的组件都提供纯文字版变体。\n" +
