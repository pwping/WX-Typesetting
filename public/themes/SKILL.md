...
- **封面图的唯一合法来源**：Markdown 的**第一个内容元素就是图片**（图片上方没有任何文字、标题、引言等）时，该图作为封面图；否则本文**没有封面图**。
- **没有封面图时**：封面组件一律使用**无图版**（删除封面中的图片槽位/`<figure>`/`<img>`），**绝对禁止**用占位图 URL（如 `placehold.orence.net`）、自创图片 URL 或正文图片充当封面图。
- **有封面图时**：封面使用该图；该图作为封面后，正文开头不再重复出现它（同一 URL 若在正文其他位置再次出现，仍按正文图片保留）。其余正文图片一律按原文位置正常保留，不受封面规则影响。
...
1. **章节自动编号**：**严格按 Markdown 中 `##` 出现的先后顺序分配 `01/02/03…`**，从第一个 `##` 开始连续递增，**绝不能跳号、绝不能根据章节内容含义判断**。末章编号变体（`///`、`∞`、`LAST` 等）**只用于 Markdown 中实际最后一个 `##` 标题**，仅看位置不看内容含义——即使某个 `##` 标题的字面包含「结语/总结/写在最后」字眼，只要它后面还有其他 `##` 标题，就只能使用数字编号，不能使用末章变体。**每个 `##` 标题都是独立的章节，绝不能合并相邻的 `##` 标题**（例如 `## 数据` + `## 隐私` 必须保留为两个独立章节，不能合并成"数据隐私"或"隐私数据"），**也不能省略任何一个 `##` 标题**。
   - **章节标题文字来源**：**Markdown 里已有的 `##` 标题，标题文字必须原样使用，一字不改**（只允许全角化标点、添加编号与英文标签等装饰层），**禁止自行总结、改写、润色或重拟标题**——作者的标题就是作者的标题。
   - **Markdown 里没有 `##` 标题时**：才允许按当前思路，**总结分析该段内容自行生成二级标题**，保证长文有清晰章节结构。
...
5. **目录提取**：主题库有目录组件时，**目录条目文字必须取正文 `##` 二级标题**——直接用章节标题原文，禁止自创"看点名"；若该标题超过 5 个字，把标题**压缩/概括为 5 字以内的短标题**显示（保留核心含义）。数量按各主题目录组件约定（精选 3 个或全章节）。
...
10. **分行分段保真**：**原文的行/段结构必须原样保留，禁止自动整合、合并或拆分**——Markdown 中每个空行分隔的段落 → 独立的一个 `<p>` 块，**绝不能把相邻段落合并成一个大段**；段落内部的换行/分行 → 在 `<p>` 内用 `<br>` 保持断行，**不能把多行硬拼成一行**。标题、列表、引用、代码块各自的区块边界同样严格保持，不得吞并或扩张。
11. **总结性内容的呈现位置**：总结/概述**可以有，但永远不能用正文 `<p>` 呈现**——正文段落必须逐字来自原文。
   - 总结性的开场白：用**引言卡组件**（oneliner-card / 编者按 / 摘要条等），**必须放在最前面**，不得作为正文段落插在正文中间；
   - 段后一两句总结/摘要：用**摘要类组件**（oneliner-card、对比摘要卡、摘要横幅条等）单独成块，不得写成正文段落；
   - 正文里出现的每个 `<p>` 都必须在原文有对应段落，否则违规。
...
- **总结/概述放错位置（高频）**：总结性内容**可以有，但永远只能以非正文组件呈现，绝不能作为正文段落混入正文流**——
  - 总结性的开场白：用引言卡组件（oneliner-card / 编者按 / 摘要条等），**必须放在最前面**，不得作为正文 `<p>` 插在正文中间；
  - 段后一两句总结/摘要：用摘要类组件（如 oneliner-card、对比摘要卡、摘要横幅条等）单独成块，**不得写成正文段落**。
  - **正文段落本身只能来自原文，逐字转写**：原文没有的段落，一律不得以正文 `<p>` 形式出现。
- **分行分段被自动合并**：原文里空行隔开的分段必须各自独立成 `<p>`，段内换行用 `<br>` 保持——**不要"顺手"把相邻段落拼成一段，也不要把多行硬并成一行**；原文怎么分行分段，产物就怎么分行分段。
- **占位图残留**：签名区/CTA 组件里若带名片图占位（如 `<img src="...名片或引导图URL">`），没有真实图片 URL 时整行删掉，不要把占位符留在产物里。
- **目录标题必须来自二级标题**：目录/导读条目的文字**只能取正文 `##` 二级标题原文**（超过 5 字压缩为 5 字以内的短标题），**不能自创"本文看点/核心要点"式的概括**——读者应能在目录和正文标题之间对上号。
- **目录是精选不是全量**：精选式目录组件（红白/石墨/禅白）展示 3 个条目，不是完整章节列表；章节多于 3 个时挑最重要的 3 个，不要硬塞或误导读者以为只有 3 章。
