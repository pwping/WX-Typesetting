...
3. **正文关键词下划线（核心特色，严格限量）**：对**每个正文段落**最多标 1 个最重要的短语（每段不超过 1 个），**全文总计不超过 5 个**，用该主题的下划线 CSS（见 theme-index）标记。优先标核心观点、结论、关键数据、专有名词；短语 4–15 字；整段无要点可不标。即使原文没有任何加粗也要按此限量标注——它是最基础的标记，但必须克制。
4. **引言关键词高亮**：识别开头金句里的核心词，用高亮组件标记。
5. **目录提取**：从所有 `##` 取全部标题作为导读/目录要点（主题库有目录组件时），有几个章节就显示几个，不得精选不得跳过。
...
| 标记层 | 正文关键词，每段 ≤ 1 处、全文 ≤ 5 处 | 克制使用 | 下划线标记 |
...
- **下划线：严格限量**。每段最多 1 个短语，全文总计不超过 5 个。不要整段划线（失去焦点），也不要有的段标有的段漏。
...
- **目录项与章节一一对应**：前言导读/目录组件展示**全部章节**，有几个 ## 章节就显示几个目录项，不得精选、不得跳过；目录项顺序与章节顺序完全一致。
